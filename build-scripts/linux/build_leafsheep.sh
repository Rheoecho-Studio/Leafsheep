...
# This probably isn't the best way to determine if we are in the Leafsheep root directory but it works for now.
if [ ! -f aclocal.m4 ]; then
    echo "This script must be ran from the root of the Leafsheep codebase."
    exit
fi

DOCKER_DEV_NAME="leafsheep_builder"
...
if [ $d -eq 0 ]; then
  ...
  docker run -t \
    ...
    oraclelinux:8 \
    env SETUP_BUILD_IMAGE=1 /share/build-scripts/linux/build_leafsheep_subscripts/run_inside_docker.sh
...
docker run -t \
  ...
  -e GROUPNAME=$(id -gn) \
  $DOCKER_DEV_NAME \
  env MOZ_NOSPAM=1 /share/build-scripts/linux/build_leafsheep_subscripts/run_inside_docker.sh
