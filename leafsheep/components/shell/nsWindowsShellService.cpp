...
//   are mapped like so:
//
//   HKCU\SOFTWARE\Classes\.<ext>\      (default)         REG_SZ     LeafsheepHTML
//
//   as aliases to the class:
//
//   HKCU\SOFTWARE\Classes\LeafsheepHTML\
...
//
//   HKCU\SOFTWARE\Classes\LeafsheepURL\  (default)         REG_SZ     <appname> URL
...
//   -------------------------------------------------
//   The following keys are set to make Leafsheep appear in the Start Menu as the
//   browser:
//   
...
#define APP_REG_NAME L"Leafsheep"
...
// Leafsheep is the default browser for file handlers since other applications
...
// handler to another application and still have Leafsheep check if it is the
...
static SETTING gSettings[] = {
  ...
  { MAKE_KEY_NAME1("LeafsheepHTML", SOC), VAL_OPEN, OLD_VAL_OPEN },

  // Protocol Handler Class - for Windows 7 and above
  { MAKE_KEY_NAME1("LeafsheepURL", SOC), VAL_OPEN, OLD_VAL_OPEN },
...
// since they are only checked when Leafsheep is the default browser and if they
// are incorrect they are fixed without notifying the user.
static SETTING gDDESettings[] = {
  // File Handler Class
  { MAKE_KEY_NAME1("Software\\Classes\\LeafsheepHTML", SOD) },

  // Protocol Handler Class - for Windows 7 and above
  { MAKE_KEY_NAME1("Software\\Classes\\LeafsheepURL", SOD) },
             ...
             LPCWSTR aClassName)
{
  ...
  LPCWSTR progID = isProtocol ? L"LeafsheepURL" : L"LeafsheepHTML";
 ...
 * This only checks for LeafsheepURL and if aCheckAllTypes is set, then
 * it also checks for LeafsheepHTML.  Note that those ProgIDs are shared
 * by all Leafsheep browsers.
*/
bool
                                        ...
                                        bool* aIsDefaultBrowser)
{
  ...
  *aIsDefaultBrowser = true;
  ...
  // that was used to launch Leafsheep which is not necessarily a long path.
  ...
  for (; settings < end; ++settings) {
    ...
    if (REG_FAILED(res) ||
      ...
      if (REG_FAILED(res)) {
        ...
        // application when setting Leafsheep as the default browser.
        *aIsDefaultBrowser = false;
        return NS_OK;
      }
  ...
  // Only check if Leafsheep is the default browser on Windows 7 and above if the
  // previous checks show that Leafsheep is the default browser.
  if (*aIsDefaultBrowser) {
  ...
  // account didn't perform a Leafsheep update this will check if Leafsheep is the
  // default browser and if dde is disabled for each handler
  // and if it isn't disable it. When Leafsheep is not the default browser the
  ...
  if (*aIsDefaultBrowser && aForAllTypes) {
    ...
    // update it using the helper application when setting Leafsheep as the
    // default browser.
    if (REG_FAILED(res)) {
      *aIsDefaultBrowser = false;
    }
                                            ...
                                            int32_t aPosition)
{
  ...
  // eventually, the path is "%APPDATA%\Mozilla\Leafsheep\Desktop Background.bmp"
