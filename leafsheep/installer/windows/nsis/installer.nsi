...
Section "-InstallStartCleanup"
  ...
  ${CleanUpdateDirectories} "Mozilla\Leafsheep" "Mozilla\updates"
...
Section "-Application" APP_IDX
  ...
  ; On install always add the LeafsheepHTML and LeafsheepURL keys.
  ; An empty string is used for the 5th param because LeafsheepHTML is not a
  ; protocol handler.
  ${GetLongPath} "$INSTDIR\${FileMainEXE}" $8
  StrCpy $2 "$\"$8$\" -osint -url $\"%1$\""

  ; In Win8, the delegate execute handler picks up the value in LeafsheepURL and
  ; LeafsheepHTML to launch the desktop browser when it needs to.
...
Section "-InstallEndCleanup"
  ...
  ${Unless} ${Silent}
    ...
    ${If} "$0" == "1"
      ...
      ${If} "$0" != ""
        ClearErrors
        WriteRegStr HKCU "Software\Mozilla\Leafsheep" "OldDefaultBrowserCommand" "$0"
      ${EndIf}
    ...
    ${ElseIfNot} ${Errors}
      ...
      ClearErrors
      WriteRegStr HKCU "Software\Mozilla\Leafsheep" "DefaultBrowserOptOut" "True"
      ${If} ${Errors}
...
Function preSummary
  ...
  ${Unless} ${Errors}
    DeleteRegValue HKLM "Software\Mozilla" "${BrandShortName}InstallerTest"
    ; Check if Leafsheep is the http handler for this user.
    SetShellVarContext current ; Set SHCTX to the current user
    ${IsHandlerForInstallDir} "http" $R9
    ${If} $TmpVal == "HKLM"
      SetShellVarContext all ; Set SHCTX to all users
    ${EndIf}
    ; If Leafsheep isn't the http handler for this user show the option to set
    ; Leafsheep as the default browser.
    ${If} "$R9" != "true"
