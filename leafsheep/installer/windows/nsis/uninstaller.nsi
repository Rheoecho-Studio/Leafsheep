...
Section "Uninstall"
  ...
  ${un.CleanUpdateDirectories} "Mozilla\Leafsheep" "Mozilla\updates"
  ...
  ${un.RegCleanAppHandler} "LeafsheepURL"
  ${un.RegCleanAppHandler} "LeafsheepHTML"
  ${un.RegCleanProtocolHandler} "ftp"
  ${un.RegCleanProtocolHandler} "http"
  ${un.RegCleanProtocolHandler} "https"

  ClearErrors
  ReadRegStr $R9 HKCR "LeafsheepHTML" ""
  ; Don't clean up the file handlers if the LeafsheepHTML key still exists since
  ; there should be a second installation that may be the default file handler
  ${If} ${Errors}
    ${un.RegCleanFileHandler}  ".htm"   "LeafsheepHTML"
    ${un.RegCleanFileHandler}  ".html"  "LeafsheepHTML"
    ${un.RegCleanFileHandler}  ".shtml" "LeafsheepHTML"
    ${un.RegCleanFileHandler}  ".xht"   "LeafsheepHTML"
    ${un.RegCleanFileHandler}  ".xhtml" "LeafsheepHTML"
    ${un.RegCleanFileHandler}  ".oga"  "LeafsheepHTML"
    ${un.RegCleanFileHandler}  ".ogg"  "LeafsheepHTML"
    ${un.RegCleanFileHandler}  ".ogv"  "LeafsheepHTML"
    ${un.RegCleanFileHandler}  ".pdf"  "LeafsheepHTML"
    ${un.RegCleanFileHandler}  ".webm"  "LeafsheepHTML"
  ${EndIf}
  ...
  ; installing even if there is another install of Leafsheep that is set as the
  ...
  ${If} $R9 == "false"
    ...
    DeleteRegKey HKLM "$0"
    DeleteRegKey HKCU "$0"
    StrCpy $0 "Software\Classes\MIME\Database\Content Type\application/x-xpinstall;app=Leafsheep"
    DeleteRegKey HKLM "$0"
    DeleteRegKey HKCU "$0"
  ${Else}
    ReadRegStr $R1 HKLM "$0" ""
  ...
  ; If Leafsheep.exe was successfully deleted yet we still need to restart to
  ; remove other files create a dummy Leafsheep.exe.moz-delete to prevent the
  ...
  ; Users who uninstall then reinstall expecting Leafsheep to use a clean profile
  ; may be surprised during first-run. This key is checked during startup of Leafsheep and
  ; subsequently deleted after checking. If the value is found during startup
  ; the browser will offer to Reset Leafsheep. We use the UpdateChannel to match
  ; uninstalls of Leafsheep-release with reinstalls of Leafsheep-release, for example.
  WriteRegStr HKCU "Software\Mozilla\Leafsheep" "Uninstalled-${UpdateChannel}" "True"
