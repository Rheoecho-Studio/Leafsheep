...
!macro PostUpdate
  ...
  ; start menu tile.  In case there are 2 Leafsheep installations, we only do
  ; this if the application being updated is the default.
  ReadRegStr $0 HKCU "Software\Microsoft\Windows\Shell\Associations\UrlAssociations\http\UserChoice" "ProgId"
  ${If} $0 == "LeafsheepURL"
  ${AndIf} $9 != 0 ; We're not running in session 0
    ReadRegStr $0 HKCU "Software\Classes\LeafsheepURL\shell\open\command" ""
  ...
  ; Windows taskbar and start menu link maintenance
...
!macro AddAssociationIfNoneExist FILE_TYPE
  ...
  ${If} ${Errors}
    WriteRegStr SHCTX "SOFTWARE\Classes\${FILE_TYPE}"  "" "LeafsheepHTML"
  ${EndIf}
  WriteRegStr SHCTX "SOFTWARE\Classes\${FILE_TYPE}\OpenWithProgids" "LeafsheepHTML" ""
!macroend
!define AddAssociationIfNoneExist "!insertmacro AddAssociationIfNoneExist"

; Adds the protocol and file handler registry entries for making Leafsheep the
...
!macro SetHandlers
  ...
  ; Associate the file handlers with LeafsheepHTML
  ReadRegStr $6 SHCTX "$0\.htm" ""
  ${If} "$6" != "LeafsheepHTML"
    WriteRegStr SHCTX "$0\.htm"   "" "LeafsheepHTML"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.html" ""
  ${If} "$6" != "LeafsheepHTML"
    WriteRegStr SHCTX "$0\.html"  "" "LeafsheepHTML"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.shtml" ""
  ${If} "$6" != "LeafsheepHTML"
    WriteRegStr SHCTX "$0\.shtml" "" "LeafsheepHTML"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xht" ""
  ${If} "$6" != "LeafsheepHTML"
    WriteRegStr SHCTX "$0\.xht"   "" "LeafsheepHTML"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xhtml" ""
  ${If} "$6" != "LeafsheepHTML"
    WriteRegStr SHCTX "$0\.xhtml" "" "LeafsheepHTML"
  ${EndIf}
  ...
  ; An empty string is used for the 5th param because LeafsheepHTML is not a
  ; protocol handler
  ${AddDisabledDDEHandlerValues} "LeafsheepHTML" "$2" "$8,1" \
                                 "${AppRegName} HTML Document" ""

  ${AddDisabledDDEHandlerValues} "LeafsheepURL" "$2" "$8,1" "${AppRegName} URL" \
                                 "true"
...
!macro SetStartMenuInternet RegKey
  ...
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".htm"   "LeafsheepHTML"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".html"  "LeafsheepHTML"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".shtml" "LeafsheepHTML"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xht"   "LeafsheepHTML"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xhtml" "LeafsheepHTML"

  WriteRegStr ${RegKey} "$0\Capabilities\StartMenu" "StartMenuInternet" "$R9"

  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "ftp"    "LeafsheepURL"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "http"   "LeafsheepURL"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "https"  "LeafsheepURL"
...
; The IconHandler reference for LeafsheepHTML can end up in an inconsistent state
; due to changes not being detected by the IconHandler for side by side
; installs (see bug 268512). The symptoms can be either an incorrect icon or no
; icon being displayed for files associated with Leafsheep (does not use SHCTX).
!macro FixShellIconHandler RegKey
  ClearErrors
  ReadRegStr $1 ${RegKey} "Software\Classes\LeafsheepHTML\ShellEx\IconHandler" ""
  ${Unless} ${Errors}
    ReadRegStr $1 ${RegKey} "Software\Classes\LeafsheepHTML\DefaultIcon" ""
    ${GetLongPath} "$INSTDIR\${FileMainEXE}" $2
    ${If} "$1" != "$2,1"
      WriteRegStr ${RegKey} "Software\Classes\LeafsheepHTML\DefaultIcon" "" "$2,1"
    ${EndIf}
  ${EndUnless}
!macroend
...
!macro SetUninstallKeys
  ...
  ${If} $2 == ""
    ...
    ${WriteRegStr2} $1 "$0" "Publisher" "Leafsheep Development Team" 0
...
; to LeafsheepHTML when there is a value for PersistentHandler or by removing the
...
!macro FixBadFileAssociation FILE_TYPE
  ...
  ${If} "$2" != ""
    ; Since there is a persistent handler remove LeafsheepHTML as the default
    ; value from both HKCU and HKLM if it set to LeafsheepHTML.
    ${If} "$0" == "LeafsheepHTML"
    ...
    ${EndIf}
  ${ElseIf} "$0" == "LeafsheepHTML"
    ; Since KHCU is set to LeafsheepHTML remove LeafsheepHTML as the default value
...
!macro UpdateProtocolHandlers
  ...
  ${IsHandlerForInstallDir} "LeafsheepHTML" $R9
  ${If} "$R9" == "true"
    ; An empty string is used for the 5th param because LeafsheepHTML is not a
    ; protocol handler.
    ${AddDisabledDDEHandlerValues} "LeafsheepHTML" "$2" "$8,1" \
  ...
  ${IsHandlerForInstallDir} "LeafsheepURL" $R9
  ${If} "$R9" == "true"
    ${AddDisabledDDEHandlerValues} "LeafsheepURL" "$2" "$8,1" \
                                   "${AppRegName} URL" "true"
  ${EndIf}
...
!macro RemoveDeprecatedKeys
  ...
  DeleteRegKey HKLM "Software\Classes\Leafsheep.URL"
  DeleteRegKey HKCU "Software\Classes\Leafsheep.URL"
  ...
  ; Delete gopher from the user's UrlAssociations if it points to LeafsheepURL.
...
; Adds a pinned shortcut to Task Bar on update if this
  ...
  ${If} ${FileExists} "$0"
    ...
    ${If} ${Errors}
      ...
      ${If} $AddTaskbarSC == ""
        ...
        ${If} ${AtMostWin2008R2}
          ; Check if the Leafsheep is the http handler for this user
...
; Adds a pinned Task Bar shortcut if there isn't one for the main
...
!macro IsFirewallSvcRunning
  ...
  ${If} $R6 != 0
    ; MpsSvc is the Firewall service on modern Windows versions.
...
; plugin. This is a function instead of a macro so it is easily called from
; an elevated instance of the binary. Since this can be called by an
; elevated instance logging is not performed in this function.
Function SetAsDefaultAppUserHKCU
  ...
  ; Only register as the handler if the app registry name exists under the
  ; RegisteredApplications registry key. The protocol and file handlers
  ; set previously at the user level will associate this install as the
  ; default browser.
  ClearErrors
